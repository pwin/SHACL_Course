...
Two things to know about the headline. **The shapes count is the number of shapes that compiled**, node shapes and property shapes together; if it is smaller than you expect, something did not compile as a shape (s63). And **Conforms means nothing that counts was found**, which is also what it says when nothing was checked (s09). Violations, warnings and infos all count, as the specification has it, unless the report narrows that with `sh:conformanceDisallows` (s03); this engine counted violations alone before 0.3.0.

The report names focus nodes by IRI, but the diagram and the subject list do not, to begin with. **Show labels**, beside the subject filter, and **Node Labels** and **Property Labels**, in the graph toolbar, are ticked when the editor opens and put each resource's `rdfs:label` or `skos:prefLabel` where its IRI would be, so `bt:shop-inkwell` in a result row is *The Inkwell* in the diagram. Untick all three to see the same names in both.
