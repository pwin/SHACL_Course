...
- [SHACL-AF §5 SHACL Functions](https://www.w3.org/TR/shacl-af/#functions)
...
- [SHACL-AF §4 Annotation Properties](https://www.w3.org/TR/shacl-af/#sparql-constraints-annotations)
- [SHACL-AF §7 Expression Constraints](https://www.w3.org/TR/shacl-af/#ExpressionConstraintComponent)
- [SHACL 1.2 Node Expressions §2 Getting started with Node Expressions](https://www.w3.org/TR/shacl12-node-expr/#getting-started)
- [SHACL 1.2 Node Expressions §4 Node Expressions Library](https://www.w3.org/TR/shacl12-node-expr/#library)
- [SHACL 1.2 SPARQL §6 SPARQL-based Node Expressions](https://www.w3.org/TR/shacl12-sparql/#sparql-node-expressions)
- [SHACL 1.2 SPARQL §7 Declaring SPARQL Functions based on Node Expressions](https://www.w3.org/TR/shacl12-sparql/#sparql-functions)
...
|---|---|---|---|---|
| SHACL Core constraint components | SHACL §4 | **runs** | s01-s29 | All of them, including sh:closed, sh:qualifiedValueShape and the property pair components. |
...
| sh:sparql constraints, $this, $PATH, ?value, ?path | SHACL §5 | **runs** | s30-s38 |  |
...
| Custom constraint components, ASK and SELECT validators | SHACL §6 | **runs** | s39-s42 |  |
...
| Result annotations, sh:resultAnnotation | SHACL-AF §4 | **silent** |  | Results appear without the annotation. |
| SHACL functions, sh:SPARQLFunction | SHACL-AF §5 | **error** | s69 | 'The custom function ... is not supported'. |
| Node expressions: sh:this, constants, [ sh:path ], filter shapes, sh:union, sh:intersection | SHACL-AF §6 | **runs** | s43-s46 | Inside rules. Anything else is an error rather than an empty result. |
| Expression constraints, sh:expression | SHACL-AF §7 | **error** |  | 'unsupported node expression' for every form but sh:this. |
| Triple rules, SPARQL rules, sh:condition, sh:order, sh:deactivated | SHACL-AF §8 | **runs** | s43-s52 | One pass as specified; 'rules, iterated' repeats to a fixpoint, up to ten rounds. |
...
| The shnex: node expression library | SHACL 1.2 Node Expressions §4 | **error** |  | Not implemented; the vocabulary changed between drafts in 2026. |
...
`python scripts/compare_pyshacl.py` runs eight of this course's lessons through pySHACL 0.40 as well and prints both reports side by side. The directional literals in the data (`@ar--rtl`) are stripped first, because rdflib does not read them. What it showed while this course was written:
